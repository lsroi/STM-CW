...
def get_attack_interval(attack):  # 提取攻击区间的起止索引
    heads = []
    tails = []
...
# calculate F1 scores
def eval_scores(scores, true_scores, th_steps, return_thresold=False):  # 计算不同阈值下的 F1-score
...
def eval_mseloss(predicted, ground_truth):  # 均方误差（MSE）损失
...
def get_err_median_and_iqr(predicted, groundtruth):  # 误差的中位数与四分位差（IQR）
...
def get_err_median_and_quantile(predicted, groundtruth, percentage):  # 中位数 + 百分位差（近似IQR）
...
def get_err_mean_and_quantile(predicted, groundtruth, percentage):  # 去极值均值 + 百分位差
...
def get_err_mean_and_std(predicted, groundtruth):  # 平均误差 + 标准差
...
def get_f1_score(scores, gt, contamination):  # 基于异常得分和污染率（contamination）计算F1分数
