...
class Main():
    def __init__(self, train_config, env_config, debug=False):
        ...
        if 'attack' in train.columns:
            train = train.drop(columns=['attack'])  # 训练数据仅包含正常数据，无监督学习（无标记）

        feature_map = get_feature_map(dataset)  # 获得特征列表
        fc_struc = get_fc_graph_struc(dataset)  # 获得图结构
    ...
    def run(self):
        ...
        # 评估指标。输出：F1 score、Precision、 Recall
    ...
    def get_save_path(self, feature_name=''):
        ...
        if self.datestr is None:
            ...
            self.datestr = now.strftime('%m-%d-%H-%M-%S') # 修改，避免使用 Windows 系统中的非法字符
